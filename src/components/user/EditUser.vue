...
<script>
import { ref } from 'vue';
import { mwModifyUser, mwDetailUser } from '@/api/middleware/user';
...
export default {
  ...
  setup(props) {
    ...
    const getImgPath = async (event) => {
      ...
      await uploadFile('profiles', `${getAuthFromCookie()}`, saveImg.value);
    ...
    const submitForm = async () => {
      ...
      if (!!phoneNumber.value) data.p_number = phoneNumber.value;
